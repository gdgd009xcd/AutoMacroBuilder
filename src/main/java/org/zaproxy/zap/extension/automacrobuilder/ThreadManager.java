...
public class ThreadManager {
    ...
    synchronized OneThreadProcessor getProcess(InterfaceDoActionProvider provider){
        ...
        long id = th.getId();
        long siz = pmap.entrySet().stream().filter(ent -> ent.getValue().getSequenceNo() == provider.getSequnceNo()).count();
        if(siz<MAX_PROCESS){
            thcount++;
            ...
            LOGGER4J.debug("MAX_PROCESS reached. seqno:" + provider.getSequnceNo()+" threadid:" + id + " siz:" + siz + "  this:" + this);
            List<Long> endedp = null;
            long giveuptimer = System.currentTimeMillis();
            LOGGER4J.debug("start RECYCLE LOOP time:" + giveuptimer);
            while(System.currentTimeMillis() - giveuptimer < MAX_GIVE_UP_WAIT_TIME && (endedp == null || endedp.size() <= 0)){
                endedp = pmap
                        .entrySet()
                        .stream()
                        .filter(ent -> (
                                ent.getValue().isEnd()
                                || ent.getValue().howManyWaitTime() > MAX_WAIT_TIME)
                                && ent.getValue().getSequenceNo() == provider.getSequnceNo()
                        )
                        .map(ent -> ent.getKey())
                ...
                LOGGER4J.debug("RECYCLE LOOP id:" + id + " lapse:" + (System.currentTimeMillis() - giveuptimer) + " starttime:" + giveuptimer);
            ...
            if(endedp==null ||  endedp.size() <= 0){
                Optional<Long> optval = pmap.entrySet().stream().map(ent->ent.getKey()).findFirst();
                oldid = optval.get();
            ...
            try {
                ...
                LOGGER4J.debug("selected recycle object id:" + oldp.getid()+" status:" 
            ...
            if(!p.isEnd()){
                p = new OneThreadProcessor(this,th, provider);
                LOGGER4J.debug("NEWLY CREATED. seqno:" + p.getSequenceNo() + " old is runnig end id:" + oldid + "->" + id);
            }else{
                p.setNewThread(th, provider);
                LOGGER4J.debug("RECYCLED. seqno:" + p.getSequenceNo() + " end id:" + oldid + "->" + id);
            }
            pmap.remove(oldid);
            pmap.put(id, p);
            if(LOGGER4J.isDebugEnabled()) d_setMaxPmap(pmap.size());
    ...
    synchronized boolean endProcess(OneThreadProcessor p, InterfaceDoAction action, boolean doendaction){
        ...
        try {
            ...
            p.Ended();
            if(!aborted){
                LOGGER4J.debug("endProcess OK id:" + p.getid() + " WASINTERRUPED:" + (p.wasInterrupted()?"YES":"NO") );
            }else{
                LOGGER4J.warn("endProcess ABORTED WASINTERRUPTED:" + (p.wasInterrupted()?"YES":"NO") + " id:" + p.getid() );
            }
    ...
    public OneThreadProcessor getProcessWithId(long thid){
        Optional<OneThreadProcessor> opt = pmap.entrySet().stream().filter(ent -> ent.getKey() == thid).map(ent -> ent.getValue() ).findFirst();
    ...
    synchronized private void dummyinternalwaiter(){
        try {
