...
public class LockInstance {
    ...
    synchronized public long lock() {
        ...
        // At this point locked == false, but because it is synchronized, no other thread can call this method.
        locked = true;
        lockerthreadid = tid;
    ...
    synchronized public void unlock(long  lockerthread){
        long thisid = Thread.currentThread().getId();
        if(locked){
            if (lockerthread != this.lockerthreadid) {
                LOGGER4J.error("unlock warning: " + "lockthread[" + lockerthread + "]!=" + "lockerid[" + this.lockerthreadid + "] thread:" + thisid);
            }
