...
public class ParmGenCookieManager {
    ...
    private URI getURI(String domain, String path, boolean isSSL){
        try {
            ...
            URI uri = new URI(url);
            //System.out.println("getURI: [" + uri.toString() + " scheme[" +  uri.getScheme() +  "] host[" + uri.getHost() + "] path[" + uri.getPath() + "]");
            return uri;
    ...
    public List<HttpCookie> parse(String domain, String path, String cheader){
        ...
        if(parsedcookies!=null&&parsedcookies.size()>0){
            URI uri = getURI(domain,"/",true);// SSL attribute is ignored when cookie values ​​are added to this cookie store.
                                              // Cookie値がこのCookieストアに追加される際、ここで指定したSSL属性は無視されます。
            // * The domain specified in the domain attribute must be a domain containing the current host name, so, only same as host or subdomain can be specified.
    ...
    public List<HttpCookie> get(String domain, String path, boolean isSSL){
        try {
            URI uri = getURI(domain,path,isSSL);// isSSL : secure attribute is ignored in this get method.
    ...
    /**
     * getURIs always return [http://domain]. protocol always [http], path is "".
     * The value returned by the getURIs function is different from when it was added to the store.
     * so I think cookiestore's URI is something customized.
     * 
     * @return List<URI>
     */
